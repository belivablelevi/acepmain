// Updated Challenges with Imgur Upload (No Firebase Storage needed)
let currentChallenges = [];
let currentFilter = 'all';
let selectedChallenge = null;

// Same load/render functions as before
...
// UPDATED: Handle submission with Imgur upload
async function handleSubmission(e) {
  ...
  try {
    ...
    // Option 1: Base64 (for small images under 1MB)
    const photoData = [];
    for (let i = 0; i < photoFiles.length; i++) {
      ...
      progressBar.style.width = `${((i + 1) / photoFiles.length) * 80}%`;
    }
    ...
    const submissionData = {
      ...
      status: 'pending',
      photos: photoData, // Base64 encoded images
